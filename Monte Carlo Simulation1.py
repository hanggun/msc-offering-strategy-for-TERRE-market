        ...
        for i in range(t):
            for j in range(n):
                ...
                for k in range(n):
                    index += 1
                    z = n*i + k
                    if price_BM[x,y] >= price_BM[z,y] and price_BM[x,y] > price_DA[i]:
                        M_up[index, 0] = i+1
                        M_up[index, 1] = j+1
                        M_up[index, 2] = k+1
                        M_up[index, 3] = 1     
...
def plot_price(idx_day, save=True):
    ...
    plt.plot(list(range(1,25)), day_DA)
    plt.plot(list(range(1,25)), day_BM, 'r')
    plt.ylim([-20,50])
    plt.legend(['day-ahead market price', 'balancing market price'], loc='lower right')
    plt.xlabel('hour/h')
    plt.ylabel('price/£')
    if save:
        plt.savefig('day' + str(idx_day+1))
    plt.show()
...
def plot_hourly(idx_day, save=True):
    ...
    if idx_day == 2:
        ...
        for i in day_DA:
            if i <= 0:
                color.append('Negative balancing market price')
            else:
                color.append('Positive balancing market price')
    else:
        color = []
        for i in revenue_day[:,idx_day]:
            if i <= 2600:
                color.append('Common hour')
            else:
                color.append('Peak hour')
    
    plt.figure(figsize=[12, 6])
    sns.barplot(list(range(1,25)),revenue_day[:,idx_day],
    ...
    plt.ylabel('profit/£')
    if save:
        plt.savefig('hourly' + str(idx_day+1))
        
def plot_thermal(idx_day, save=True):
    plt.plot(list(range(0,23)), d[idx_day])
    plt.xlabel('hour/h')
    plt.ylabel('quantity/MWh')
    if save:
        plt.savefig('thermal' + str(idx_day+1))
        
def plot_storage(idx_day, save=True):
    plt.plot(list(range(0,23)), l[idx_day])
    plt.xlabel('hour/h')
    plt.ylabel('quantity/MWh')
    if save:
        plt.savefig('storage' + str(idx_day+1))
...
def plot_daily(save=True):
    ...
    plt.figure(figsize=[12, 6])
    sns.barplot(list(range(1,31)), x, dodge=False, hue=color, palette=['#3498db', "#e74c3c"])
    plt.legend()
    plt.xlabel('day')
    plt.ylabel('profit/£')
